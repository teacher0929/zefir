                ...
                <form action="{{ route('admin.products.update', $obj->id) }}" method="post" enctype="multipart/form-data">
                            ...
                            @foreach($categories as $grandparent)
                                @foreach($grandparent->children as $parent)
                                    @foreach($parent->children as $child)
                                        <option value="{{ $child->id }}" {{ $child->id == $obj->category_id ? 'selected':'' }}>
                        ...
                        <div class="col">
                            ...
                            <select class="form-select @error('brand') is-invalid @enderror" id="brand" name="brand" required>
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->id }}" {{ $brand->id == $obj->brand_id ? 'selected':'' }}>
                                        {{ $brand->getName() }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col">
                            ...
                            <select class="form-select @error('color') is-invalid @enderror" id="color" name="color" required>
                                @foreach($colors as $color)
                                    <option value="{{ $color->id }}" {{ $color->id == $obj->color_id ? 'selected':'' }}>
                                        {{ $color->getName() }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-4">
                            ...
                            <input type="text" class="form-control @error('group_id') is-invalid @enderror" id="group_id"
                                   name="group_id" value="{{ $obj->group_id }}" required>
                            @error('group_id')
                            <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror"
                                  name="description" id="description" rows="2" required>{{ $obj->description }}</textarea>
                        @error('description')
                        <div class="alert alert-danger mt-2">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col">
                            ...
                            <input type="number" step="0.1" min="0"
                                   ...
                                   name="discounted_price" value="{{ $obj->discounted_price }}" required>
                        ...
                        <div class="col">
                            ...
                            <input type="number" step="0.1" min="0"
                                   ...
                                   name="selling_price" value="{{ $obj->selling_price }}" required>
                            @error('selling_price')
                            <div class="alert alert-danger mt-2">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('sizes') is-invalid @enderror" id="sizes" name="sizes[]" multiple size="3" required>
                            @foreach($sizes as $size)
                                <option value="{{ $size->id }}" {{ $obj->variants->pluck('size_id')->contains($size->id) ? 'selected':'' }}>
                                    {{ $size->getName() }}
                                </option>
                            @endforeach
                        </select>
